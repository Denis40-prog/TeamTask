<div class="max-w-7xl mx-auto p-4 sm:p-6">
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-2xl font-semibold">Suivi — {{ $team->name }}</h1>
        <a href="{{ route('wellness.followup') }}" class="text-sm text-indigo-600">← Mes équipes</a>
    </div>

    <p class="text-sm text-gray-500 mb-6">Vue des 30 derniers jours. Clique un graphique pour voir le détail d’un membre.</p>

    <div class="space-y-6">
        @foreach ($members as $m)
            @php $uid = $m['id']; @endphp
            <div class="rounded-2xl border bg-white dark:bg-gray-900 p-4">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <a href="{{ route('wellness.followup.team.member', [$team->id, $uid]) }}"
                       class="text-sm text-indigo-600 hover:underline">Détails →</a>
                </div>

                <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                    @foreach (['sleep' => 'Sommeil 😴','stress'=>'Stress 🧠','soreness'=>'Courbatures 💪','energy'=>'Énergie ⚡️'] as $metric => $title)
                        <div class="rounded-xl border p-3 hover:shadow transition cursor-pointer"
